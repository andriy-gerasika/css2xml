...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="css2xml">
					...
					<xsl:choose>
						...
						<xsl:when test="regex-group(9)">
							<xsl:analyze-string select="regex-group(9)"
								regex="^({string-join($config/keyword, '|')})$|^({string-join($config/value, '|')})$|^({string-join($config/font, '|')})$|^((-?\d+)(\.\d+)?)$" flags="si">
								<xsl:matching-substring>
									<xsl:choose>
										<!-- keyword -->
										<xsl:when test="regex-group(1)">
		...
		<xsl:variable name="mode5">
			<xsl:apply-templates mode="css2xml5" select="$mode4"/>
		</xsl:variable>
		<xsl:copy-of select="$mode5"/> <!-- change $mode0 to $mode[0-9] for easy debug -->
	</xsl:template>
	...
	<xsl:template mode="css2xml2" match="selectors/keyword | selectors/value">
		<name>
			<xsl:value-of select="."/>
		</name>
	</xsl:template>
	...
	<xsl:template mode="css2xml3" match="selector/name[not(preceding-sibling::node()[1]/self::symbol)]">
		<element>
			<xsl:apply-templates mode="css2xml3" select="@*"/>
			<xsl:attribute name="name" select="."/>
		</element>
	</xsl:template>

	<xsl:template mode="css2xml3" match="selector/symbol[.=('#', '.', ':') and following-sibling::node()[1]/self::name]"/>

	<xsl:template mode="css2xml3" match="selector/name[preceding-sibling::node()[1]/self::symbol[.='#']]">
		<element>
			<xsl:apply-templates mode="css2xml3" select="@*"/>
	...
	<xsl:template mode="css2xml3" match="selector/name[preceding-sibling::node()[1]/self::symbol[.=':']]">
		<element>
			<xsl:apply-templates mode="css2xml3" select="@*"/>
			<xsl:attribute name="pseudo" select="."/>
		</element>
	</xsl:template>
	...
	<xsl:template priority="-9" mode="css2xml5" match="@*|node()">
		<xsl:copy>
			<xsl:apply-templates mode="css2xml5" select="@*|node()"/>
		</xsl:copy>
	</xsl:template>

	<xsl:template mode="css2xml5" match="property/node()"/>
